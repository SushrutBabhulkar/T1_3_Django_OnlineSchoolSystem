...
def homework(request):
    subjects = Subjects.objects.filter(staff_id=request.user.id)
    session_years = Courses.objects.all()
    context = {
        "subjects": subjects,
        "session_years": session_years
    }
    if request.method=="POST":
        ...
        ins=Homework(subject,classes,question)
        ins.save()
    return render(request, "staff_template/assign_homework.html", context)
...
def get_classes(request):
    ...
    classes = Courses.objects.filter(course_id=subject_model.course_id, session_year_id=session_model)

    # Only Passing Student Id and Student Name Only
    list_data = []

    for student in classes:
        data_small={"id":Courses.admin.id, "name":Courses.admin.course_name}
        list_data.append(data_small)

    return JsonResponse(json.dumps(list_data), content_type="application/json", safe=False)

@csrf_exempt
def save_class_data(request):
    ...
    json_student = json.loads(course_ids)
    # print(dict_student[0]['id'])

    # print(student_ids)
    try:
        # First Attendance Data is Saved on Attendance Model
        attendance = Homework(subject_id=subject_model, attendance_date=attendance_date, session_year_id=session_year_model)
        attendance.save()

        for stud in json_student:
            # Attendance of Individual Student saved on AttendanceReport Model
            student = Students.objects.get(admin=stud['id'])
            attendance_report = Homework(student_id=student, attendance_id=attendance, status=stud['status'])
